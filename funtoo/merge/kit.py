...
async def checkout_kit(hub, kit_dict=None, pull=None):
	...
	out_tree = git_class(hub, kit_dict["name"], branch=branch, root=root, **kwargs)
	out_tree.initialize()
...
async def generate_kit(hub, kit_dict=None):
	...
	if kit_dict["kind"] == "independent":
	...
	elif kit_dict["kind"] == "autogenerated":
		steps += [CleanTree()]
		...
		steps += [
			RemoveFiles(hub.merge.foundations.get_excludes_from_yaml(kit_dict["name"])),

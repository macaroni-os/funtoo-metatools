...
class KitGenerator:
	...
	def get_source_repo(self):
		"""
		When auto-generating a sourced repository, this will return a single reference to the repository, so we can
		reference repo.tree when copying things from it.
		"""
		if self.kit.kind != KitKind.SOURCED:
			raise TypeError("get_source_tree() must not be used with auto-generated kits.")
	...
	def packages_yaml_steps(self):
		...
		"""
		if self.kit.kind != KitKind.AUTOGENERATED:
			raise TypeError("packages_yaml_steps() must not be used with non-autogenerated kits.")
		steps = []

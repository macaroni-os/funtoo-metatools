...
async def release_gen(hub, github_user, github_repo, release_data=None, tarball=None):
	"""
...
def iter_tag_versions(tags_list):
	"""
...
async def latest_tag_version(hub, github_user, github_repo, tag_data=None):
	"""
	...
	if tag_data is None:
		tag_data = await hub.pkgtools.fetch.get_page(f"https://api.github.com/repos/{github_user}/{github_repo}/tags", is_json=True)
	versions_and_tag_elements = list(iter_tag_versions(tag_data))
...
async def tag_gen(hub, github_user, github_repo, tag_data=None):
	"""
	...
	"""
	result = await latest_tag_version(hub, github_user, github_repo, tag_data=tag_data)
	if result is None:
		return None

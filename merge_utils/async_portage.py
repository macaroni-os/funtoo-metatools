...
async def async_xmatch(self, level, origdep, mydep=None, mykey=None, mylist=None):
	...
	if myval is not None:
		...
		for cpv in iterfunc(mylist):
			...
			try:
				pkg_str = _pkg_str(cpv, metadata=metadata, settings=self.settings, db=self)
			except InvalidData:
				continue
		...
		warnings.warn(
			"The 'bestmatch-list' mode of " "portage.dbapi.porttree.portdbapi.xmatch is deprecated",
			DeprecationWarning,
			stacklevel=2,
		)
		...
		warnings.warn(
			"The 'match-list' mode of " "portage.dbapi.porttree.portdbapi.xmatch is deprecated",
			DeprecationWarning,
			stacklevel=2,
		)

...
class SourceCollection:
	...
	def initialize(self):
		...
		self.yaml.model.log.info(f"Initializing source collection {self.name} with {len(self.repositories)} repositories")
...
class Kit:
	...
	def __init__(self, locator, release=None, name=None, stability=None, branch=None, eclasses=None, priority=None, aliases=None, masters=None, sync_url=None, settings=None):
...
class ReleaseYAML(YAMLReader):
	...
	def _source_collections(self):
		...
		for collection_name, collection_items in self.iter_groups("release/source-collections"):
			...
			source_collections[collection_name] = SourceCollection(yaml=self, name=collection_name, repositories=collection_objs)
	...
	def _kits(self):
		...
		for kit_el in self.iter_list("release/kit-definitions/kits"):
			...
			if kind == KitKind.AUTOGENERATED:
				...
				kits[kit_name].append(AutoGeneratedKit(locator=self.model.locator, release=self, name=kit_name, **kit_insides))
